...
Client.prototype._processMessage = function (msg) {
	...
	switch (code) {
		...
		case "250":
			//console.log( "MSG:", msg );
			if( msg === "2.0.0 Ready to start TLS" ) {
				var self = this;
				this.socket = this.sock;
				this.sock = tls.connect( {socket:this.socket} );
				this.sock.on( "secureConnect", function( a ){
					//console.log( "THIS?", a, this );
					self.conversationStep++;
					self.sock.write("EHLO " + os.hostname() + "\r\n");
				});
				this.sock.on( "error", (err)=>{ console.log( "error evnet..", err ) } );
				this.sock.on( "close", ()=>{ console.log( "close evnet.." ) } );
				this.sock.on("data", function (data) {
					//console.log( "data on tls", data );
					self.buffer += data;
					self._checkBuffer();
				});
				return;
